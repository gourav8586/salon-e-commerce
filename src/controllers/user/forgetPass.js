...
exports.otpVerify = async (req, res) => {
  try {
    ...
    console.log("user email for otp verify----" + userData);
...
exports.change_pass = async (req, res) => {
  try {
    ...
    let userDetails = await user.findOne({ email: email }); // Changed from admin to user
    let user_email = userDetails.email;
    let passwordUpdate = await user.findOneAndUpdate( // Changed from admin to user
      { email: user_email },
      { $set: { pass: hashedPassword } }
    );
    if (passwordUpdate) {
